...
function NodeSvc(config){
	...
	try{
		...
		log.info("Error reading config file with exception:"+e);		
		log.info("Creating default conf file.");
		this.conf = {"log" : "false"};
	}	
}
...
NodeSvc.prototype.startServices = function startServices(){
	if(!utils.isEmpty(this.conf) && !utils.isEmpty(this.conf.services) && this.conf.services.length>0){												
		//loop through the services defined in config and auto start them.
		async.forEach(this.conf.services, startService, function(err){
			if(err)
				log.error(process.title+": Error auto starting services with error: "+err);
			else{
				log.info(process.title+": Finished auto starting service(s) - "+JSON.stringify(this.conf.services));
			}
		});	
	}				
}//startServices			
